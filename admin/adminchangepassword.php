...
<?php include '../classess/AdminChanges.php';?>
<?php
    $AdminChanges = new AdminChanges();
?>
<?php 
$msg= "";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
	...
	if(!empty(trim($newpass)) || !empty(trim($confirmpass))){
		...
		if($newpass == $confirmpass){
			...
			$adch = $AdminChanges->changeAdminForgotPassword($newpass,$code,$email);
			
		// delete column from tbl_emailgencode table
			$AdminChanges->deleteGencode($email);
		}
		else{
...
if(isset($_GET['code'])){
	...
	$check = $AdminChanges->checkCode($code);
