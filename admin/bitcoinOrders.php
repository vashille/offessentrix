...
if (isset($_GET['shiftid'])) {
	$id = $_GET['shiftid'];
	$shift = $bitcoin->productShiftedFromBitcoin($id);
...
if (isset($_GET['delproid'])) {
	$id = $_GET['delproid'];
		...
		$archive->insertToArchive($id,"Bitcoin");
	
	
	$delOrder = $bitcoin->delProductShiftedFromBitcoin($id);

}
 ?>
        <div class="grid_10">
            <div class="box round first grid">
                <h2>Bitcoin Orders</h2>
                <?php 
                if (isset($shift)) {
                	echo $shift;
                }
                ...
                <div class="block">        
					...
					<tbody>
						...
						$getOrder = $bitcoin->getAllBitcoinOrderProduct();
						if ($getOrder) {
						...
						<tr class="odd gradeX">
							...
							if ($result['status'] == '0') { ?>
							<td><a href="?shiftid=<?php echo $result['id']; ?>">Shipped out</a></td>	
							<?php }elseif($result['status'] == '1'){?>
								<td><a href="?shiftid=<?php echo $result['id']; ?>">Pending</a></td>
							<?php } else{ ?>
